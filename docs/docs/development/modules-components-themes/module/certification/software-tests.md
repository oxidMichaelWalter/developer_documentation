...
If there are, use stubs and dependency injection. For more informtion, see the PHPUnit Manual under [Test Doubles](https://docs.phpunit.de/en/10.2/test-doubles.html).
...
Stick to the [structure example given in modules/structure](../skeleton/structure.md).

### OXID test folder usage

Find sample tests under [Module Template repository on GitHub](https://github.com/OXID-eSales/module-template).

## Running tests, creating and reading reports

### Running tests

See [testing sections](../../testing/index.md)
...
Use the [OXID eShop Docker SDK](https://github.com/OXID-eSales/docker-eshop-sdk) to generate a clean OXID eShop environment (with testing tools).

1. Install the desired shop version and edition (see, for example, the recipes under [github.com/OXID-eSales/docker-eshop-sdk-recipes](https://github.com/OXID-eSales/docker-eshop-sdk-recipes)).
2. Install your module according to its installation instructions.
3. Run all shop and module tests.
4. Ensure that all tests pass. If any shop tests fail, be prepared to provide explanations.

:::note todo
OXDEV-9255: check the revided instruction above --HR 2025-04-08: OXVM is not used for a long time, please verify that docker sdk is mentioned in this documentation: zum Testen recipes benutzen; reuse blogpost text;
where do I find "reuse blogpost text"
:::