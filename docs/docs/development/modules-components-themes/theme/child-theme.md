...
Besides completely custom themes you can also create child themes that inherit from a parent theme. This gives you the opportunity to modify only the parts you desire, while the rest stays the same. Please keep in mind that there's only one layer of inheritance with child themes. This means you can extend one installed parent theme with a child but not the child with another child again.

## composer.json

A child theme is an extension like a normal theme, module or component and must be encapsulated in a Composer package. The whole process to generate a Composer installable theme is described in the [previous section](theme-via-composer). We focus on the must-have contents here:
...
If you wish, you can also add theme settings as usual.
...
This means we must copy the exact same structure in our child theme for the template `header.html.twig`:
...
If we activate our child theme now, the template `header.html.twig` from CHILD is taken while the template `footer.html.twig` is still taken from APEX. This means we only copy and modify the templates we desire.
...
![Twig child theme example](/media/screenshots/twig_child_theme_001.png)
...
While `logo.svg` is now loading from CHILD all other assets still come from APEX.
...
If you are in development phase and run `composer update` remember to answer the questions to overwrite files in `source` with yes. Otherwise your changes from your child theme will not be transferred to `Application/views` and `out` directory.
